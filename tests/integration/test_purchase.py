class TestPurchase:
    ...
    def test_purchase_places_with_valid_request(
            self, client, login_as_valid_user):
        """
        Case 1 — valid purchase:
        Confirmation message displayed and points/places deducted.
        """
        login_as_valid_user()
        ...
        competition = next(
            c for c in client.application.config['COMPETITIONS']
            if c['name'] == 'Fall Classic'
        )
        club = next(
            c for c in client.application.config['CLUBS']
            if c['name'] == 'Simply Lift'
        )
        assert competition['number_of_places'] == '8'
        assert club['points'] == '8'

    def test_purchase_places_when_competition_is_complete(
            self, client, login_as_valid_user):
        """
        Case 2 — competition complete:
        Message indicating that the competition is no longer open.
        """
        login_as_valid_user()
        competition = next(
            c for c in client.application.config['COMPETITIONS']
    ...
    def test_purchase_places_more_than_available_places(
            self, client, login_as_valid_user):
        """
        Case 3 — request exceeds available places:
        Denial with explicit message.
        """
        login_as_valid_user()
        competition = next(
            c for c in client.application.config['COMPETITIONS']
        ...
        assert response.status_code == 200
        assert (
            b"Not enough places available in this competition."
            in response.data
        )
        assert competition['number_of_places'] == '3'

    def test_purchase_places_more_than_twelve(
            self, client, login_as_valid_user):
        """
        Case 4 — request exceeds 12 places:
        Denial to ensure fairness.
        """
        # If done in multiple times, the club can book more than 12 places,
        ...
        assert response.status_code == 200
        assert (
            b"You cannot book more than 12 places per competition."
            in response.data
        )
    ...
    def test_purchase_places_more_than_club_points(
            self, client, login_as_valid_user):
        """
        Case 5 — request exceeds club points:
        Denial with explicit message.
        """
        login_as_valid_user()
        club = next(
        ...
        assert response.status_code == 200
        assert (
            b"Not enough points available in your club to book the "
            b"requested number of places."
            in response.data
        )
        assert club['points'] == '4'

    def test_purchase_multiple_times_accumulates_points_and_places(
            self, client, login_as_valid_user):
        """
        Case 6 — multiple purchases:
        Points and places are correctly updated after several purchases.
        """
        login_as_valid_user()
        # Premier achat
    ...
    def test_purchase_12_places_in_multiple_requests(
            self, client, login_as_valid_user):
        """
        Case 7 — booking 12 places in multiple requests:
        The club cannot book more than 12 places in total.
        """
        login_as_valid_user()
    ...
    def test_purchase_requires_login(self, client):
        ...
        Purchase denied and redirect to index.
        """
        response = client.post(
            '/purchase_places',
            data={
